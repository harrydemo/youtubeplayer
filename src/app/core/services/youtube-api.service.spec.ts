...
describe('YoutubeApiService', () => {
  ...
  it('should set configuration when instansiated', () => {
    // harry spyOn(YoutubeApiService.prototype, 'setConfig').and.callThrough();
    const options = {
      url: 'mocked-url',
      http: {},
      idKey: 'mocked-idkey',
    ...
    const service = new YoutubeApiService(options, authSpy);
    // harry const actual = service.setConfig;
    const expected = 1;
    // harry expect(actual).toHaveBeenCalledTimes(expected);
  });
